    ...
    return (
        <>
            ...
            <div className='my-4 p-4 m-4 overflow-hidden'>
                <Slider {...settings} activeIndex={index} onSelect={handleSelect}>
                    {productsData.filter((item) => item.tag === 'featured-product').map((item) => (
                            <div key={item.id} className='slide-item'>
                                <div className='product'>
                                    <p>{item.title}</p>
                                    <img src={item.images[0]} alt={item.title} 
                                    className='img-fluid m-4' style={{ height: '150px' }}/>
                                    <div className='product-info'>
                                        <div className='d-flex align-items-center mt-6 m-4'>
