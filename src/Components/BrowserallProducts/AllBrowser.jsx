  ...
  function AllBrowser() {
    ...
    const lowest = () => {
      setData([...allProducts].sort((First, Second) => First.finalPrice - Second.finalPrice));
      setActiveSort('lowest'); 
    };

    const highest = () => {
      setData([...allProducts].sort((First, Second) => Second.finalPrice - First.finalPrice));
      setActiveSort('highest'); 
    };

    const handleBrandChange = (brand) => {
      setSelectedBrands(prev => 
        prev.includes(brand) ? prev.filter(brand => brand !== brand) : [...prev, brand]
      );
    };
              ...
              {initialData.map((products) => (
                <div key={products.id} className='col-md-3 mb-4'>
                  ...
                  <div className='card bg-black text-white border p-2'>
                      ...
                      <div className='card-body'>
                      <Link to={`/product/${products.id}`} className='text-decoration-none'>
                        <h3 className='card-title text-truncate text-white pe-4'>{products.title}</h3>
                        <p className='card-text text-truncate text-white fw-bolder'>{products.info}</p>
                        </Link>
                        <hr className='pt-2 pb-2' />
                        <div className='d-flex align-items-center'>
                          <h3 className='card-title text-truncate p-2 mb-0'>₹{products.finalPrice}</h3>
